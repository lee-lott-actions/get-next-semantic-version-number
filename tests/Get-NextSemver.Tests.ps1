...
Describe "Get-NextSemver" {
    BeforeAll {
        $script:owner = "dummy-owner"
        $script:repo = "dummy-repo"
        $script:branch = "main"
        $script:token = "dummy-token"
        $script:useLeadingZeros = $true
        $script:numberOfLeadingZeros = 4
    }
    ...
    Context "Success Cases" {
        It "unit: Get-NextSemver writes outputs for stable release" {
        ...
        It "unit: Get-NextSemver writes outputs for pre-release version" {
            Mock Get-LastStableTag {
                [PSCustomObject]@{ Success = $true; Tag = "v1.2.3"; Error = $null }
            } -ModuleName Get-NextSemver
            Mock Get-CommitsSince {
        ...
        It "unit: Get-NextSemver writes outputs for no changes since last release" {
            Mock Get-LastStableTag {
                [PSCustomObject]@{ Success = $true; Tag = "v1.2.3"; Error = $null }
            } -ModuleName Get-NextSemver
            Mock Get-CommitsSince {
        ...
        It "unit: Get-NextSemver handles first release (no stable tag found)" {
            Mock Get-LastStableTag {
    ...
    Context "Failure Cases" {
        It "unit: Get-NextSemver writes result=failure if Get-LastStableTag fails" {
            Mock Get-LastStableTag {
        ...
        It "unit: Get-NextSemver writes result=failure if Get-CommitsSince fails" {
            Mock Get-LastStableTag {
                [PSCustomObject]@{ Success = $true; Tag = "v1.2.3"; Error = $null }
            } -ModuleName Get-NextSemver
            Mock Get-CommitsSince {
        ...
        It "unit: Get-NextSemver writes result=failure if Get-NextVersion fails" {
            Mock Get-LastStableTag {
                [PSCustomObject]@{ Success = $true; Tag = "v1.2.3"; Error = $null }
            } -ModuleName Get-NextSemver
            Mock Get-CommitsSince {
    ...
    Context "Parameter Valdiation Failure Cases" {
        It "unit: Get-NextSemver with empty isPrerelease" {
            Get-NextSemver `
                -isPrerelease "" `
                -prereleaseId "dev" `
                -owner $owner `
        ...
        It "unit: Get-NextSemver with empty prereleaseId" {
            Get-NextSemver `
                -isPrerelease "true" `
                -prereleaseId "" `
                -owner $owner `
        ...
        It "unit: Get-NextSemver with empty owner" {
            Get-NextSemver `
                -isPrerelease "true" `
                -prereleaseId "dev" `
                -owner "" `
        ...
        It "unit: Get-NextSemver with empty repo" {
            Get-NextSemver `
                -isPrerelease "true" `
                -prereleaseId "dev" `
                -owner $owner `
        ...
        It "unit: Get-NextSemver with empty branch" {
            Get-NextSemver `
                -isPrerelease "true" `
                -prereleaseId "dev" `
                -owner $owner `
        ...
        It "unit: Get-NextSemver with empty token" {
            Get-NextSemver `
                -isPrerelease "true" `
                -prereleaseId "dev" `
                -owner $owner `
